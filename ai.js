// 刻度 · 外部接口：Claude（食物识别 / 周计划 / 周报）与 Open Food Facts（条码）
// 浏览器直连 Anthropic API，仅供个人使用；API key 存本机 localStorage。
window.KD_AI = (() => {
...
async function call(apiKey, body) {
  ...
  const r = await fetch(API, {
    ...
    body: JSON.stringify({ model: MODEL, max_tokens: 16000, thinking: { type: 'adaptive' }, output_config: { effort: 'medium' }, ...body }),
  });
  if (!r.ok) {
...
async function recognizeFood(apiKey, base64, mediaType) {
  ...
  return call(apiKey, {
    system: '你是营养师。识别照片中的每一种食物/菜品，用中文命名，估计份量与每份热量和三大营养素（克，整数）。中式菜按常见做法估算油量。无法确定时给出最可能的判断，并在 alternatives 里给 2-3 个相近选项。只输出 JSON。',
    messages: [{ role: 'user', content: [
      { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
      { type: 'text', text: '识别这张照片里的食物。' },
    ] }],
    output_config: { effort: 'medium', format: schema({ items }, ['items']) },
  });
}

// 周计划：让模型在动作库范围内生成，保证解析/视频可用
async function generatePlan(apiKey, ctx) {
  ...
  return call(apiKey, {
    ...
    output_config: { effort: 'high', format: schema({ days: { type: 'array', items: day }, note: { type: 'string', description: '一句话说明本周安排思路' } }, ['days', 'note']) },
  });
}

// 周报：结论 + 三条下周建议
async function weeklyReview(apiKey, ctx) {
  return call(apiKey, {
    system: '你是减脂教练。根据本周数据写周报：headline 是一两句直接结论（不超过 40 字，不客套）；tips 为 3 条具体可执行的下周建议，每条不超过 30 字。中文，只输出 JSON。',
    messages: [{ role: 'user', content: JSON.stringify(ctx) }],
    output_config: { effort: 'medium', format: schema({ headline: { type: 'string' }, tips: { type: 'array', items: { type: 'string' } } }, ['headline', 'tips']) },
  });
}
...
return { recognizeFood, generatePlan, weeklyReview, lookupBarcode };
})();

...
步骤：1) 先找参照物判断尺度：普通饭碗口径约 11–12cm、盛满米饭约 150–200g；外卖餐盒一格米饭约 200–250g；筷子长约 24cm；餐盘直径约 20–26cm；易拉罐 330ml。2) 逐个食物判断名称（用最常见的通用菜名）、可见部分被遮挡的比例、堆叠高度，估算克数。3) 给出每 100g 的营养密度（按常见做法含油量）。4) 混合菜（盖浇饭、便当、炒饭）拆成主食 + 菜分别列出。5) 汤只算汤本身，饮料按体积估克数。6) 认出连锁快餐（沙县小吃、兰州拉面、麦当劳、肯德基、瑞幸等）时用该店的菜名（如 "花生酱拌面" "无皮扁食" "蒸饺"），份量按该店标准份估。

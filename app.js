...
function rProfile() {
  ...
  <div class="scroll" style="padding:0 20px">
    ...
    ${row('重新走一遍引导', '›', 'restart')}
    ${row('清空全部数据', '›', 'wipe')}
    <div class="muted" style="font-size:11px;padding:16px 0 24px" data-act="reloadApp">刻度 v14 · 数据只存在这台手机的浏览器里 · 点此检查更新</div>
  </div>${nav()}</div>`;
}
...
function rWorkout() {
  ...
  <div class="scroll" style="display:flex;flex-direction:column;justify-content:center;padding:0 20px">
    ...
    ${a.setIdx === 0 && !a.sets.some(x => x.ex === e.name) ? `<div data-act="exDetail" data-name="${esc(e.name)}" style="margin-top:10px;border:1px solid var(--line);padding:10px 12px;display:flex;gap:12px;align-items:center"><div style="width:44px;height:44px;flex:none;background:${info.video ? 'var(--acc)' : 'var(--surface)'};color:#fff;display:flex;align-items:center;justify-content:center">${I.play.replace('<svg', '<svg width="20" height="20"')}</div><div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:800">${info.video ? '示范视频 · 动作解析 ›' : '动作解析 ›'}<span class="muted" style="font-weight:400;margin-left:6px">${info.m}</span></div><div class="muted" style="font-size:12px;line-height:1.5;margin-top:3px">${info.tips.slice(0, 2).map(t => '· ' + esc(t)).join('　')}</div></div></div>` : ''}
